...
Если OpenRouter включен, для проектов, прошедших фильтры, бот может сгенерировать
...
- `Короче`: сделать более короткий вариант
- `Добавить вопросы`: добавить уточняющие вопросы
- `Демо`: появляется только если модель считает, что по описанию заказа можно собрать осмысленный мини-прототип
- `Отправил вручную`: отметить, что отклик отправлен вами на Kwork

Во время генерации отклика модель дополнительно оценивает, достаточно ли данных в заказе
для демо. Если да, по кнопке `Демо` бот собирает небольшой zip-архив демо-проекта и
отправляет его в Telegram.

Локально демо-проекты сохраняются в:
...
В чат можно отправить команду:

```text
/health
```

Бот ответит кратким состоянием: доступность процесса, путь к базе, количество проектов,
счетчики статусов, feedback и последнюю сохраненную ошибку. В групповом чате команда
обрабатывается только из `TELEGRAM_CHAT_ID`, указанного в конфиге.
